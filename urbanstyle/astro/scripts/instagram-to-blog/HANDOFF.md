...
| `llm.ts` | Gemini (primary, model chain 3.6→3.5→2.5-flash for quota 429) + DeepSeek (direct API `deepseek-flash` = V4.1 Flash by default; `LLM_PROVIDER=opencode` switches back to the zen/go gateway). `generateArticles` (dual), `generateTextCompletion` (raw text), `groundedCompletion` (Google Search grounding), bounded 503/429 availability retry (Gemini 503: 30s→60s then falls back; 429 advances the model chain), describeImage (Gemini vision), guillemets «» → italic `*"..."*` normalization |
...
- LLM: `LLM_PROVIDER=deepseek` (direct API, default; set `opencode` for the zen/go gateway fallback), `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL=deepseek-flash` (V4.1 Flash; `DEEPSEEK_BASE_URL` optional, defaults to `https://api.deepseek.com`); legacy gateway block: `OPENCODE_API_KEY` (opencode-go), `OPENCODE_BASE_URL=https://opencode.ai/zen/go/v1`, `OPENCODE_MODEL=deepseek-v4-flash`; `GEMINI_API_KEY`, `GEMINI_MODEL=gemini-3.6-flash`, `GEMINI_MODELS=gemini-3.6-flash,gemini-3.5-flash,gemini-2.5-flash`
...
- Gemini free tier: 20 req/day per model → model chain fallback essential; grounding uses 2.5-flash; `withAvailability` and `withAvailability503` are BOUNDED so a quota-exhausted/503 day fails fast → callers fall back to DeepSeek / repair path instead of blocking forever. Gemini call sites pass `retry503:false` (the inner chain already retried 503; retrying again would multiply the wait)
