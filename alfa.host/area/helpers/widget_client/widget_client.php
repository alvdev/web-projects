...
class WidgetClient extends AbstractWidget
{
    ...
    public function startBody($output = true)
    {
        ...
        if ($panel_nav != '') {
            $html .= '
                <div class="card-nav mt-8 flex justify-between items-center">
                    ' . $panel_nav . '
    ...
    private function buildWidgetButtons()
    {
        ...
        if (!empty($this->widget_buttons)) {
            $html .= '<div class="btn-group float-right border-2 border-yellow-500">';
    ...
    private function buildLinkButtons(array $attributes = null)
    {
        ...
        if (!empty($this->link_buttons)) {
            $html = '<div class="flex items-center min-w-fit gap-4 *:hover:cursor-pointer">' . $this->eol;
