            ...
            <?php foreach ($page->find('book')->images() as $image): ?>
                <div class="relative flex gap-8">
                    <picture>
                        ...
                        <img
                            alt="<?= $image->alt() ?>"
                            src="<?= $image->resize(175)->url() ?>"
                            srcset="<?= $image->srcset() ?>"
                            sizes="<?= $sizes ?>"
                            width="<?= $image->resize(175)->width() ?>"
                            height="<?= $image->resize(263)->height() ?>"
                            loading="lazy" class="w-full h-auto rounded-xl ring-2 ring-indigo-900/50 aspect-2/3 object-cover saturate-110 contrast-110">
                    </picture>
