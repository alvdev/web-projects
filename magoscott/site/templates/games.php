    ...
    <section id="intro" class="relative pt-28 md:pt-48 bg-linear-to-bl from-indigo-900 via-indigo-950 via-50% to-black">
        <div class="container relative w-full flex flex-col md:flex-row md:flex-wrap items-center gap-8 lg:gap-16 justify-center text-xl md:text-2xl lg:text-3xl">
            <?php foreach ($games as $game): ?>
                <button x-on:click="selectedTab = '<?= $game->slug() ?>'" x-bind:class="selectedTab === '<?= $game->slug() ?>' ? 'bg-white text-black' : ''" class="md:min-w-1/3 inline-block font-semibold text-shadow-2xs text-shadow-black uppercase border-2 hover:bg-white hover:text-indigo-950 rounded-4xl px-8 py-3 text-balance w-fit max-w-[23ch] md:max-w-none"><?= $game->title() ?></button>
            <?php endforeach ?>
        </div>
    </section>
        ...
        <?php foreach ($games as $game): ?>
            <div x-show="selectedTab === '<?= $game->slug() ?>'" x-transition x-cloak>
                <section class="bg-linear-to-tl from-red-600 via-indigo-950 via-50% to-black">
                    <div class="container pt-20 lg:pt-24 xl:pt-28 flex flex-col lg:flex-row items-start gap-8 md:gap-16 lg:gap-32">
                        <div class="w-full lg:w-1/2">
                            <hgroup class="*:[h1]:text-4xl md:*:[h1]:text-7xl *:[h1]:font-bold *:[h1]:leading-none *:[h1]:text-balance">
                                <h1><?= $game->headerTitle() ?></h1>
                            </hgroup>

                            <div class="prose prose-lg md:prose-xl *:[p]:mt-8 *:[p]:text-xl md:*:[p]:text-2xl *:[p]:text-pretty *:[p]:text-white/80 **:[a]:inline-block **:[a]:text-fuchsia-300 **:hover:[a]:text-fuchsia-500">
                                <?= $game->description() ?>
                            </div>
                        ...
                        <div class="sticky top-36 w-full lg:w-1/2">
                ...
                <?php if ($game->videoTitle()->isEmpty() == false && $game->video()->isEmpty() == false): ?>
                ...
                <?php else: ?>
                    <div class="relative z-10 -mb-14 md:-mb-16 pt-28 md:pt-32 lg:pt-36 bg-linear-to-bl from-red-600 to-indigo-950 to-50%"></div>
                <?php endif ?>
            </div>
        <?php endforeach ?>
    </div>
