...
<header id="home-header"
        class="bg-linear-to-tl from-red-600/30 via-indigo-950/30 via-50% to-black/30">
    <div
         class="absolute top-0 -z-10 w-full opacity-90 *:mask-b-to-90% *:[img]:w-full *:[img]:object-cover *:[img]:aspect-video">
        <?php if ($image = $page->cover()->toFile()): ?>
        <picture>
            <!-- Mobile: Vertical 1:3 Crop (Strict) -->
            <source media="(max-width: 600px)"
                    srcset="<?= $image->srcset('avif-mobile') ?>"
                    sizes="100vw">
            <source media="(max-width: 600px)"
                    srcset="<?= $image->srcset('webp-mobile') ?>"
                    sizes="100vw">
            ...
            <source srcset="<?= $image->srcset('webp') ?>"
                    type="image/webp"
                    sizes="(min-width: 1024px) 50vw, 100vw">
            <img srcset="<?= $image->srcset('default') ?>"
                 sizes="<?= $sizes ?>"
